...
async def get_users_by_owners(request: Request, user=Depends(auth.auth_user_wrapper), db: Session = Depends(get_session), owner_id: int = 0):
...
async def get_users_by_owners_and_status(request: Request, user=Depends(auth.auth_user_wrapper), db: Session = Depends(get_session), owner_id: int = 0, status: int = 0):
...
async def get_single_user(request: Request, user=Depends(auth.auth_user_wrapper), db: Session = Depends(get_session), user_id: int = 0):
