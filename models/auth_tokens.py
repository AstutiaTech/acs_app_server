...
class Auth_Token(Base):
    ...
    pinged_at = Column(TIMESTAMP(timezone=True), nullable=True)
...
def create_auth_token(db: Session, admin_id: int=0, user_id: int=0, token: str=None, status: int=0, expired_at: str=None):
    auth_token = Auth_Token(admin_id=admin_id, user_id=user_id, token=token, status=status, pinged_at=get_laravel_datetime(), expired_at=expired_at, created_at=get_laravel_datetime(), updated_at=get_laravel_datetime())
    db.add(auth_token)
    db.commit()
...
def ping_auth_token(db: Session, id: int = 0):
    return update_auth_token(db=db, id=id, values={'pinged_at': get_laravel_datetime()})
